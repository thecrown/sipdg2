...
$title = <<<EOD
<h1>Laporan Pertanggungjawaban Bulan $bulan $tahun </h1> 
EOD;
...
$table .= '<tr>
		...
		foreach ($data as $key) {
			...
			$jumlah = $this->Model_barang->getpenambahan2($key->id_barang);
			$pengurangan = $this->Model_barang->getpengurangan2($key->id_barang);
...
$table .='<table>';
			...
			$table .='
				...
				$hasil2 = $this->Model_barang->countPenambahanBarang2();
			$table .='
                  		<td style="border:1px solid #000";>Rp'.number_format($hasil2->Total).'</td>';
               $hasil3 = $this->Model_barang->countPenguranganBarang2();
            $table .='  		
